...
#Start od dziś - 365 dni to start
start = dt.datetime.now() - dt.timedelta(days=365)
end = dt.datetime.now()
#GSPC - SP500, YAHOOFINANCESPI, FROM START TILL END
sp500_df = web.DataReader('^GSPC', 'yahoo', start, end)
#Zmiana - użycie funkcji z panadas - Celem jest porównanie do SP500 innych stock
...
#This time we are just checking a few companies we can always check all of them  - few minutes
counter = 0

for ticker in tickers:
    df = web.DataReader(ticker, 'yahoo', start, end)
    #I like to save to csv file
...
#Below we are checking only 10 companies and break then outr calculations
    counter += 1
    if counter == 10:
        break

best_performers = pd.DataFrame(list(zip(tickers, return_list)), columns=['Ticker', 'Returns Compared'])
#Porównanie w pct = procentach true oraz razy 100 aby pokazac procenty
best_performers['Score'] = best_performers['Returns Compared'].rank(pct=True) * 100
#From that point from everything - quantile(0.8) - everyting below that is worse and we wont only TOP20%
#It will be 0.7 70% top performers compared results to sp500 results
...
for ticker in best_performers['Ticker']:
    #for these tickers we are going to use csv file
    try:
        df = pd.read_csv(f'stock_data/{ticker}.csv', index_col=0)
        moving_averages = [150, 200]
        for ma in moving_averages:
            dt['SMA_'+ str(ma)] = round(df['Adj Close'].rolling(window=ma).mean(), 2)

        latest_price = df['Adj Close'][1]
        pe_ratio = float(si.get_quote_table(ticker)['PE Ratio (TTM)'])
        #look for si.get_stats - peg_ratio is at [1][4] location - always look for it in docs
        ...
        #Get that instance where score value is ticker get the score value and then turn it to the list and pick it
        ...
        if condition_1:
                #and condition_2 and condition_3 and condition_4 and condition_5:
